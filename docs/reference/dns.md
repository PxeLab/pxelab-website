...
- **本地解析** — A / AAAA / CNAME / TXT / MX 记录
- **上游转发** — 配置上游 DNS 后，未匹配的查询转发到上游
...
| 上游 DNS | 空（不转发） | 逗号或空白分隔的多个 `host:port`，如 `8.8.8.8:53 1.1.1.1:53`；留空则仅应答本地记录，不向上游转发 |

> **注意**：上游 DNS 默认为空，即默认**不转发**任何查询，仅返回本地记录。如需转发，请在配置中显式填写上游地址。

Web UI：**服务配置 → DNS**（`/services/dns`）
