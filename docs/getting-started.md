...
1. **创建接口与子网**：进入 **基础配置 → 服务配置 → DHCP**，点击「新增接口」，设置网段（如 `192.168.50.0/24`）、地址池、网关、DNS，DHCP 模式保持 **server**，保存后启动 DHCP 服务。
2. **开启 OS 安装目录（Netboot）**：进入 **基础配置 → 服务配置 → OS 安装目录**，打开「启用」开关（**默认关闭**，需手动开启）。
3. **客户端开机**：进入启动菜单（F12 / F11 / Esc），选择**网络引导（PXE Boot）**。
