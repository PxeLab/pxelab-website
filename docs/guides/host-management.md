...
- 机房批量上线 → 通过 [自动化 API](../development/automation.md) 循环创建
...
批量登记多台主机没有内置 CSV 导入；可通过 [自动化与 CI 集成](../development/automation.md) 的 REST API 循环创建。
...
有引导记录的主机在详情页显示「在线」。仪表盘的「在线主机」统计则取最近 10 分钟内有引导记录的机器。列表还显示引导次数与最近在线时间，用于确认装机/引导是否成功。
